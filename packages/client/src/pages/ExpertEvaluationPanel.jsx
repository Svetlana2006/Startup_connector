...
const VALIDATION_METRICS = {
  ...
  11: [
    ...
    },
    {
      ...
      verified: "4",
      status: "warning",
      note: "2 pilots pending evidence",
    },
    {
      label: "Core team size",
      claimed: "18",
      verified: "18",
      status: "match",
      note: "Payroll records confirmed",
    },
  ],
};
...
export default function ExpertEvaluationPanel({ user, onLogout }) {
  ...
  const activePanel =
    evaluationPanels.find((p) => p.panelId === selectedPanelId) ||
    evaluationPanels[0];
  const currentEvaluatorRecord = activePanel.evaluators.find(
    (e) => e.evaluatorId === activeEvaluatorId,
  );
  ...
  const [comments, setComments] = useState(
    currentEvaluatorRecord?.comments || "",
  );
  ...
  function handleSubmitScore() {
    ...
    triggerToast(
      `Independent evaluation submitted by ${currentEvaluatorRecord?.name || "Evaluator"}!`,
    );
  }

  function handleAssignEvaluator() {
    const ev = SEED_EVALUATORS_POOL.find(
      (e) => e.evaluatorId === newEvaluatorId,
    );
    if (!ev) return;
  ...
  const submittedCount = activePanel.evaluators.filter(
    (e) => e.submitted,
  ).length;
  const totalEvaluators = activePanel.evaluators.length;
  const allSubmitted =
    activePanel.allSubmitted || submittedCount === totalEvaluators;
  const validationMetrics =
    VALIDATION_METRICS[activePanel.startupId] || VALIDATION_METRICS[11];
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-md mb-lg">
            <div>
              ...
              <p className="text-body-md font-body-md text-on-surface-variant">
                Admin multi-evaluator assignment with blind review protocol.
                Consensus revealed only after all evaluators submit.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const p = evaluationPanels.find(
                    (x) => x.panelId === e.target.value,
                  );
                  if (p?.evaluators?.[0]) {
              ...
              >
                {evaluationPanels.map((p) => (
                  <option key={p.panelId} value={p.panelId}>
                    {p.panelId}: {p.startupName} (
                    {p.allSubmitted ? "✅ Finalized" : "⏳ In Progress"})
                  </option>
                ))}
              </select>
            </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-sm pb-md border-b border-outline-variant mb-md">
                <div>
                  ...
                  <p className="text-label-sm text-on-surface-variant mt-0.5">
                    Problem Challenge:{" "}
                    <strong>{activePanel.problemTitle}</strong> (
                    {activePanel.dept})
                  </p>
                </div>
                  ...
                  <div>
                    {allSubmitted ? (
                      ...
                      <span className="inline-flex items-center gap-1 px-3 py-1 bg-warning-container text-on-warning-container rounded-full text-label-sm font-bold">
                        ...
                        Blind Review Active ({submittedCount}/{totalEvaluators}{" "}
                        Submitted)
                      </span>
                    )}
                  </div>
                    ...
                    >
                      ...
                      <div className="mt-2 pt-2 border-t border-outline-variant/50 flex justify-between items-center text-[11px]">
                        <span className="text-on-surface-variant">
                          {ev.submitted
                            ? `Score submitted ${ev.submittedDate || ""}`
                            : "Awaiting rubric"}
                        </span>
                        <button
                          onClick={() => {
            ...
            <div className="lg:col-span-4 bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md md:p-lg flex flex-col justify-between">
              <div>
                ...
                <p className="text-label-sm text-on-surface-variant leading-relaxed">
                  Under MSInS evaluation bylaws, peer evaluator scores and
                  comments remain cryptographically encrypted and hidden from
                  other panel members until{" "}
                  <strong>100% of assigned evaluators have submitted</strong>.
                </p>
                <div className="mt-3 p-3 bg-surface-container-low rounded-lg border border-outline-variant text-[11px] space-y-1">
                  ...
                  <p className="text-on-surface-variant">
                    • Prevents anchor bias during technical review
                  </p>
                  <p className="text-on-surface-variant">
                    • Automatically calculates arithmetic mean across 6 rubric
                    criteria
                  </p>
                  <p className="text-on-surface-variant">
                    • Feeds final aggregate score into Pipeline Dashboard
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-outline-variant">
                <span className="text-label-sm text-on-surface-variant">
                  Active Scoring Perspective:
                </span>
                <p className="font-bold text-primary text-label-md flex items-center gap-1 mt-0.5">
                  <span className="material-symbols-outlined text-[18px]">
                    badge
                  </span>
                  {currentEvaluatorRecord?.name || "Evaluator"} (
                  {currentEvaluatorRecord?.institution})
                </p>
              </div>
            </div>
          </div>

          {/* Independent Validation View */}
          <section
            className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md md:p-lg mb-lg"
            aria-labelledby="validation-heading"
          >
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-sm pb-md border-b border-outline-variant mb-md">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="material-symbols-outlined text-secondary">
                    fact_check
                ...
                <p className="text-label-sm text-on-surface-variant mt-1">
                  Compare {activePanel.startupName}&apos;s submitted claims with
                  department evidence before scoring.
                </p>
              </div>
              <span className="inline-flex items-center gap-1.5 self-start px-3 py-1.5 rounded-full bg-secondary-container text-on-secondary-container text-label-sm font-bold">
                <span className="material-symbols-outlined text-[16px]">
                  shield
                </span>
                Evidence review
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
              <div className="rounded-lg border border-primary/20 bg-primary/5 overflow-hidden">
                <div className="p-md border-b border-primary/10 flex items-center gap-2">
                  <span className="material-symbols-outlined text-primary">
                    assignment
                  </span>
                  <div>
                    <h3 className="text-label-md font-bold text-primary">
                      Startup Self-Reported Metrics
                    </h3>
                    <p className="text-[11px] text-on-surface-variant">
                      Values declared in the application
                    </p>
                  </div>
                </div>
              ...
              <div className="rounded-lg border border-secondary/30 bg-secondary/5 overflow-hidden">
                <div className="p-md border-b border-secondary/20 flex items-center gap-2">
                  <span className="material-symbols-outlined text-success">
                    verified_user
                  </span>
                  <div>
                    <h3 className="text-label-md font-bold text-primary">
                      Department-Verified Metrics
                    </h3>
                    <p className="text-[11px] text-on-surface-variant">
                      Cross-checked against submitted evidence
                    </p>
                  </div>
                </div>
                  ...
                  {validationMetrics.map((metric) => (
                    ...
                    >
                      ...
                      <div className="text-right shrink-0">
                        ...
                        {metric.status !== "match" && (
                          <span
                            className={`inline-flex px-2 py-0.5 mt-1 rounded text-[10px] font-bold ${
                              metric.status === "error"
            ...
            <div className="lg:col-span-8 bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md md:p-lg">
              <div className="flex justify-between items-center pb-md border-b border-outline-variant mb-md">
                <div>
                  ...
                  <p className="text-label-sm text-on-surface-variant">
                    Score each dimension from 1 (Unsatisfactory) to 10
                    (Outstanding).
                  </p>
                </div>
              ...
              <div className="space-y-md">
                {evaluationCriteria.map((crit, idx) => {
                  const scoreVal =
                    scores[crit.id] !== undefined ? scores[crit.id] : 7;
                  return (
                    <div
                      key={crit.id}
                      className="p-3 bg-surface-container-low rounded-lg border border-outline-variant/60"
                    >
                      <div className="flex justify-between items-start gap-md mb-2">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <h4 className="font-bold text-label-md text-on-surface">
                              {crit.name}
                            </h4>
                          </div>
                          <p className="text-[12px] text-on-surface-variant mt-1 pl-7">
                            {crit.description}
                          </p>
                        </div>
            ...
            <div className="lg:col-span-4 bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md md:p-lg flex flex-col">
              ...
              {!allSubmitted ? (
                <div className="flex-1 flex flex-col items-center justify-center p-md text-center bg-surface-container-low rounded-lg border border-dashed border-outline-variant">
                  ...
                  <h4 className="font-bold text-label-md text-on-surface mb-1">
                    Consensus Locked
                  </h4>
                  <p className="text-label-sm text-on-surface-variant max-w-xs">
                    Scores from all 3 evaluators must be submitted before
                    aggregated consensus is computed and revealed.
                  </p>
                ...
                <div className="space-y-md">
                  ...
                  <div className="pt-md border-t border-outline-variant">
                    <h4 className="text-label-sm font-bold text-on-surface mb-2">
                      Evaluator Remarks Summary:
                    </h4>
                    <div className="space-y-2 max-h-48 overflow-y-auto text-[12px]">
                      {activePanel.evaluators.map((e, idx) => (
                        <div
                          key={idx}
                          className="p-2 bg-surface-container-low rounded border border-outline-variant/50"
                        >
              ...
              >
                ...
                <div className="space-y-md">
                  ...
                  <p className="text-[12px] text-on-surface-variant">
                    Assigned evaluators will receive an automated notification
                    alert and rubric access link in their portal workspace.
                  </p>
