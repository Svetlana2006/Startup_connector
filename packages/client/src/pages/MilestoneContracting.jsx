...
const STATUS_CONFIG = {
  pending: { label: 'Pending', icon: 'hourglass_empty', bg: 'bg-surface-container-high', text: 'text-on-surface-variant' },
  in_progress: { label: 'In Progress', icon: 'pending', bg: 'bg-warning-container', text: 'text-on-warning-container' },
  submitted: { label: 'Submitted (Ready for Review)', icon: 'upload_file', bg: 'bg-primary-fixed text-primary', text: 'text-primary font-bold' },
  approved: { label: 'Approved & Disbursed', icon: 'verified', bg: 'bg-success text-on-primary', text: 'text-on-primary font-bold' },
  paid: { label: 'Paid (GFR Disbursed)', icon: 'payments', bg: 'bg-success text-on-primary', text: 'text-on-primary font-bold' },
}

export default function MilestoneContracting({ user, onLogout }) {
  const { milestoneContracts, updateMilestoneStatus, notifications } = useAppContext()
  ...
  const activeContract = milestoneContracts.find(c => c.pilotId === selectedPilotId) || milestoneContracts[0]

  const totalAllocated = activeContract?.totalBudget || 0
  const totalPaid = activeContract?.milestones
    ?.filter(m => m.status === 'paid' || m.status === 'approved')
    ?.reduce((sum, m) => sum + m.amount, 0) || 0
  const paidPercent = totalAllocated > 0 ? Math.round((totalPaid / totalAllocated) * 100) : 0
  ...
  function handleStatusChange(milestoneId, newStatus) {
    ...
    if (newStatus === 'submitted') {
      triggerToast(`Deliverables submitted for "${milestone?.name}". Sent to Department Official for review.`)
    } else if (newStatus === 'approved') {
      triggerToast(`Milestone approved! ₹${(milestone?.amount / 100000).toFixed(1)}L simulated disbursement executed via GFR 173(i) ledger.`)
    } else {
      triggerToast(`Milestone status updated to "${newStatus.replace('_', ' ')}".`)
    }
  }
  ...
  function handleAddMilestone(e) {
    ...
    const pct = totalAllocated > 0 ? ((amt / totalAllocated) * 100).toFixed(1) : 25
    ...
    setNewDueDate('')
    setNewAmount('')
    triggerToast(`New milestone "${newName}" added to pilot contract ${activeContract.pilotId}.`)
  }
        ...
        <main className="flex-1 md:ml-64 p-md md:p-lg max-w-[1280px] mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-md mb-lg">
            <div>
              ...
              <p className="text-body-md font-body-md text-on-surface-variant">
                Jointly define deliverables, review milestone submissions, and auto-disburse staged payments.
              </p>
            </div>
          ...
          <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md md:p-lg mb-lg">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-md pb-md border-b border-outline-variant">
              <div>
                <span className="text-label-sm text-on-surface-variant">Pilot Reference</span>
                <p className="text-headline-md font-bold text-primary">{activeContract.pilotId}</p>
                <p className="text-label-sm text-on-surface-variant">{activeContract.dept}</p>
              </div>
              <div>
                <span className="text-label-sm text-on-surface-variant">Selected Startup</span>
                ...
                <p className="text-label-sm text-on-surface-variant">DPIIT Recognized · GFR 173(i)</p>
              </div>
              <div>
                <span className="text-label-sm text-on-surface-variant">Problem Challenge</span>
                <p className="text-body-md font-bold text-on-surface truncate" title={activeContract.problemTitle}>
                  {activeContract.problemTitle}
                </p>
                <p className="text-label-sm text-secondary font-bold">Challenge ID: {activeContract.problemId}</p>
              </div>
              <div>
                <span className="text-label-sm text-on-surface-variant">Total Pilot Grant / Cap</span>
                <p className="text-headline-md font-bold text-primary">
                  ₹{(totalAllocated / 100000).toFixed(1)} Lakh
                </p>
                <p className="text-label-sm text-success font-bold">Escrow Disbursement</p>
              </div>
            </div>
            ...
            <div className="pt-md">
              <div className="flex justify-between items-center mb-1 text-label-sm font-bold">
                <span className="text-success flex items-center gap-1">
                  ...
                  Disbursed: ₹{(totalPaid / 100000).toFixed(1)} Lakh ({paidPercent}%)
                </span>
                <span className="text-on-surface-variant">
                  Remaining in Escrow: ₹{((totalAllocated - totalPaid) / 100000).toFixed(1)} Lakh
                </span>
              </div>
            ...
            <div className="p-md md:p-lg border-b border-outline-variant flex flex-col sm:flex-row justify-between items-start sm:items-center gap-md">
              <div>
                ...
                <p className="text-label-sm text-on-surface-variant">
                  Status flow: <span className="font-bold text-on-surface">Pending → In Progress → Submitted → Approved & Disbursed</span>
                </p>
              </div>
              <button
                    ...
                    return (
                      <tr key={m.id} className="hover:bg-surface-container-low/50 transition-colors">
                        ...
                        <td className="p-md max-w-md">
                          ...
                          {m.deliverableUrl && (
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {m.submittedDate && (
                                <span className="text-[11px] text-on-surface-variant">Submitted: {m.submittedDate}</span>
                              )}
                            </div>
                          )}
                        </td>
                        ...
                        <td className="p-md whitespace-nowrap">
                          <p className="font-bold text-primary text-[16px]">₹{(m.amount / 100000).toFixed(1)} Lakh</p>
                          <p className="text-[11px] text-on-surface-variant">{m.percentage}% of grant cap</p>
                        </td>
                        <td className="p-md whitespace-nowrap">
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-label-sm font-bold ${cfg.bg} ${cfg.text}`}>
                        ...
                        <td className="p-md whitespace-nowrap text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            {isInProgress && (
                              <button
                                onClick={() => handleStatusChange(m.id, 'submitted')}
                            ...
                            {(isSubmitted || (isInProgress && isDept)) && (
                              ...
                              >
                                ...
                                Approve & Disburse (₹{(m.amount / 100000).toFixed(1)}L)
                              </button>
                            )}
          ...
          <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md md:p-lg mb-lg">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-sm mb-md pb-sm border-b border-outline-variant">
              <div>
                <h3 className="text-headline-md font-headline-md text-primary flex items-center gap-2">
                  ...
                  Simulated GFR Rule 173(i) Payment Ledger ({activeContract.paymentRecords?.length || 0})
                </h3>
                <p className="text-label-sm text-on-surface-variant">
                  Audit-compliant disbursement records tied directly to verified milestone deliverables.
                </p>
              </div>
            ...
            {activeContract.paymentRecords?.length === 0 ? (
              <div className="p-md bg-surface-container-low rounded-lg text-center text-on-surface-variant text-label-md">
                No milestone payments have been released yet. Approving a milestone automatically executes simulated release.
              </div>
            ) : (
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-label-md font-bold text-primary">{pay.reference}</span>
                      ...
                      <p className="text-label-sm text-on-surface-variant mt-0.5">
                        Released to <strong>{activeContract.startupName}</strong> on {pay.date} · {pay.notes || 'Milestone approval trigger'}
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-[10px] text-on-surface-variant font-mono">ID: MSINS-PAY-00{pay.milestoneId}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
              ...
              >
                ...
                <form onSubmit={handleAddMilestone} className="space-y-md">
                  <div>
                    <label className="block text-label-sm font-bold text-on-surface mb-1">Milestone Name *</label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="block text-label-sm font-bold text-on-surface mb-1">Deliverable Scope & Criteria</label>
                    <textarea
                      rows="3"
                  ...
                  <div className="grid grid-cols-2 gap-md">
                    <div>
                      <label className="block text-label-sm font-bold text-on-surface mb-1">Due Date *</label>
                      <input
                        type="date"
                        required
                        value={newDueDate}
                    ...
                    <div>
                      <label className="block text-label-sm font-bold text-on-surface mb-1">Tranche (in ₹ Lakh) *</label>
                      <input
                        type="number"
                        step="0.1"
                        required
