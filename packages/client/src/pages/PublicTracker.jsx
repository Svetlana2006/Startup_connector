...
const ACTIVITY_LOG = [
  {
    title: 'Transitioned to Piloting', time: 'Oct 24, 2024 - 14:32',
  ...
  {
    title: 'Evaluation Completed', time: 'Oct 20, 2024 - 09:15',
  ...
  {
    title: 'Application Received', time: 'Oct 15, 2024 - 11:45',
    ...
    <div className="bg-surface text-on-surface min-h-screen flex flex-col font-body-md">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 md:ml-64 p-md md:p-lg overflow-y-auto max-w-[1280px] mx-auto w-full">
          <div className="mb-lg">
            <h1 className="text-headline-lg font-headline-lg text-on-background mb-xs">Public Status Tracker</h1>
            <p className="text-body-md font-body-md text-on-surface-variant max-w-3xl">
              Transparent lifecycle tracking for high-priority platform initiatives. All milestone transitions are cryptographically hashed for auditability.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
            ...
            <section className="lg:col-span-12 bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md lg:p-lg">
              <div className="border-b border-outline-variant pb-sm mb-md flex justify-between items-center">
                <h2 className="text-headline-md font-headline-md text-on-surface">Initiative: Smart Grid Pilot Alpha</h2>
                <span className="bg-[#e6f4ea] text-[#137333] px-sm py-xs rounded-full text-label-sm font-label-sm font-bold border border-[#ceead6] flex items-center gap-xs">
                  <span className="material-symbols-outlined text-[14px]">check_circle</span> Active Phase: Piloting
                </span>
              </div>
              {/* Horizontal pipeline */}
                ...
                <div className="relative z-10 flex justify-between items-center w-full">
                  {PIPELINE_STAGES.map((stage, i) => (
                    <div key={i} className="flex flex-col items-center gap-sm">
                      <div className={`flex items-center justify-center text-[14px] shadow-sm border-2 border-surface-container-lowest
                        ${stage.active
                        ...
                        <span className={`material-symbols-outlined ${stage.active ? 'text-[20px] fill-icon' : 'text-[16px]'}`}>{stage.icon}</span>
                      </div>
            ...
            {/* Activity Log (8 cols) */}
            <section className="lg:col-span-8 bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md lg:p-lg flex flex-col h-full">
              <div className="border-b border-outline-variant pb-sm mb-md">
                <h3 className="text-body-lg font-body-lg font-bold text-on-surface">Immutable Activity Log</h3>
                <p className="text-label-sm font-label-sm text-on-surface-variant">Cryptographically verifiable transition states.</p>
              </div>
              <div className="flex-1 overflow-y-auto pr-sm space-y-md">
                {ACTIVITY_LOG.map((log, i) => (
                  <div key={i} className="flex gap-md">
                    <div className="flex flex-col items-center">
                      <div className={`w-2 h-2 rounded-full ${log.dotColor} mt-1`} />
                      {i < ACTIVITY_LOG.length - 1 && <div className="w-px h-full bg-outline-variant my-1" />}
                    </div>
                    <div className="pb-md w-full">
                      <div className="flex justify-between items-start mb-xs">
                        <h4 className={`text-label-md font-label-md font-bold text-on-surface ${i === 0 ? 'font-bold' : ''}`}>{log.title}</h4>
                        <span className="text-label-sm font-label-sm text-on-surface-variant">{log.time}</span>
                      </div>
                      <p className="text-body-md font-body-md text-on-surface-variant mb-sm">{log.desc}</p>
                      <div className="bg-surface-container-low p-sm rounded-md font-mono text-[10px] text-on-surface-variant break-all border border-outline-variant">
                        Hash: {log.hash}
                      </div>
                    </div>
                  </div>
            ...
            <section className="lg:col-span-4 flex flex-col gap-gutter">
              ...
              <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md lg:p-lg flex flex-col">
                <div className="border-b border-outline-variant pb-sm mb-md">
                  <h3 className="text-body-lg font-body-lg font-bold text-on-surface">Scale-up Decision</h3>
                </div>
                <div className="flex-1 flex flex-col items-center justify-center py-lg">
                  ...
                  <h4 className="text-headline-md font-headline-md text-on-surface text-center mb-xs">Pending Validation</h4>
                  <p className="text-body-md font-body-md text-on-surface-variant text-center mb-md">Statewide adoption recommendation awaits pilot conclusion data.</p>
                  <div className="w-full bg-surface-container-low rounded-lg p-sm border border-outline-variant mt-auto">
                    <div className="flex justify-between items-center mb-xs">
                      <span className="text-label-sm font-label-sm text-on-surface-variant">Pilot Progress</span>
                      <span className="text-label-sm font-label-sm font-bold text-primary">65%</span>
                    </div>
              ...
              <div className="bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm p-md lg:p-lg">
                <div className="border-b border-outline-variant pb-sm mb-md">
                  <h3 className="text-label-md font-label-md font-bold text-on-surface">Pilot Metrics</h3>
                </div>
                <ul className="space-y-sm">
                  {PILOT_METRICS.map(([k, v]) => (
                    <li key={k} className="flex justify-between items-center py-xs border-b border-surface-container-high last:border-0">
                      <span className="text-label-md font-label-md text-on-surface-variant">{k}</span>
                      <span className="text-body-md font-body-md font-bold text-on-surface">{v}</span>
                    </li>
                  ))}
                </ul>
              </div>
